...
Reframing Ossified Bitcoin Quirks as Overlay Search Surfaces}
...
\begin{abstract}
Bitcoin's historical consensus envelope contains behaviors that are no longer
desirable as deployable transaction patterns but remain informative as design
motifs. This paper argues that three such motifs are especially reusable:
transcript multiplicity, identifier bifurcation, and carrier camouflage. We
present a toolchain for extracting, replaying, labeling, and classifying these
surfaces across historical Bitcoin windows, then show how the same abstractions
can be lifted into live TradeLayer Litecoin prototypes with BitVM-style dispute
flows. The contribution is not a proposal to revive old bugs. It is a
translation layer from fossilized Bitcoin behavior to modern overlay-relevant
search surfaces, together with a policy-envelope method for distinguishing
replay-only fossils from clean contemporary isomorphisms.
\end{abstract}

\section{Introduction}

Bitcoin's consensus behavior is full of inherited irregularities that were once
ordinary, later became policy-constrained, or were eventually superseded by
cleaner transaction forms. These historical quirks are often discussed as
curiosities, hazards, or museum pieces. We take a different view: some of them
encode durable protocol ideas that can inspire modern overlay design even when
the original transaction forms are no longer directly usable.
...
libertarian engineering culture: behaviors that were tolerated, under-specified,
or simply left in place long enough to ossify. That period is over as a live
design regime, but its fossils remain informative. The claim of this paper is
that some of those neglected fingerprints are still isomorphically usable as
protocol mechanics once they are abstracted away from their original,
historically contingent transaction forms.
...
We study those motifs with a deterministic observability toolchain and then lift
them into live Litecoin-based prototypes where BitVM-style routes, challenges,
...
remaining methodologically closer to differential blockchain testing than to new
consensus design \cite{linus2023bitvm,linus2026binohash,yang2021fluffy,kim2025fork}.
...
\begin{enumerate}
\item It defines a reproducible \emph{historical-to-modern} workflow for turning
Bitcoin fossils into measured experimental surfaces.
\item It organizes the current evidence into three reusable overlay-relevant
motifs rather than a loose list of quirks.
\item It demonstrates live prototype grafts on Litecoin/TradeLayer that show
how these motifs can interact with BitVM-style flow graphs and dispute edges.
...
TradeLayer Litecoin is used as a controllable live environment for prototyping
